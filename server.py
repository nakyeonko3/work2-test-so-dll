...
def handle_user():
    ...
    if EventID == "1":
        ...
        # C 프로그램 실행
        result = subprocess.run(["./Cuser_data", UserData["Name"], UserData["CarType"], UserData["CarNumber"]], capture_output=True, text=True)

        if result.returncode == 0:
            print("C 프로그램의 출력: ", result.stdout)
            return jsonify({"message": "데이터가 등록되었습니다."})
        else:
            return jsonify({"message": "C 프로그램 실행 중 오류가 발생했습니다."}), 500
